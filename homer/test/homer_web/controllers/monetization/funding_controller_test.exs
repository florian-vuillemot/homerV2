defmodule HomerWeb.Monetization.FundingControllerTest do
  ...
  def fixture(:funding) do
    {:ok, funding} = Monetization.create_funding(@create_attrs)
    funding
  end
  ...
  describe "index" do
    test "lists all fundings", %{conn: conn} do
      conn = get conn, monetization_funding_path(conn, :index)
  ...
  describe "create funding" do
    test "renders funding when data is valid", %{conn: conn} do
      conn = post conn, monetization_funding_path(conn, :create), funding: @create_attrs
      assert %{"id" => id} = json_response(conn, 201)["funding"]


      conn = get conn, monetization_funding_path(conn, :show, id)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, monetization_funding_path(conn, :create), funding: @invalid_attrs
  ...
  describe "update funding" do
    ...
    test "renders funding when data is valid", %{conn: conn, funding: %Funding{id: id} = funding} do
      conn = put conn, monetization_funding_path(conn, :update, funding), funding: @update_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, funding: funding} do
      conn = put conn, monetization_funding_path(conn, :update, funding), funding: @invalid_attrs
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete funding" do
    ...
    test "deletes chosen funding", %{conn: conn, funding: funding} do
      conn = delete conn, monetization_funding_path(conn, :delete, funding)
      assert response(conn, 204)
      assert_error_sent 404, fn ->
        get conn, monetization_funding_path(conn, :show, funding)
      end
    end
  end
